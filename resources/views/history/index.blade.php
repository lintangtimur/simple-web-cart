                        ...
                        @foreach ($tx as $trans)
                            <div class="flex flex-col border-b border-gray-300 py-2">
                                <div class="flex items-center justify-between mb-2">
                                    <p class="text-lg font-semibold">Invoice #{{$trans->id}}</p>
                                    
                                    <span class="text-sm text-gray-500">Waktu Pembelian: {{$trans->transaction_date}}</span>
                                </div>
                        
                                <!-- Table for item details -->
                                @foreach ($trans->details as $item)
                                ...
                                <div class="flex justify-end">
                                    <p class="text-lg font-semibold">Total Harga: Rp {{Number::format($trans->total_amount)}}</p>
                                </div>
                                <!-- Status information -->
                                <div class="mt-2">
                                    
                                    <span class="px-2 py-1 rounded font-semibold text-white 
                                        {{ strtotime('now') - strtotime($trans->transaction_date) > 3 * 60 * 60 ? 'bg-red-500' : 'bg-green-500' }}">
                                        {{ strtotime('now') - strtotime($trans->transaction_date) > 3 * 60 * 60 ? 'Closed' : 'Open' }}
                                    </span>
                                </div>
                            </div>    
                        @endforeach  
